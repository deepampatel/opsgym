# BanditDex State Machine

BanditDex should advance through explicit project states. Codex can orchestrate the workflow, but durable files must define what happens next.

## Session Contract

`banditdex.json` lives at the project root and captures the user's intent:
...
`.banditdex/progress.md` is append-only project memory. Record phase transitions and human decisions:
...
- `init`: create or update `banditdex.json`.
...
1. No `banditdex.json`: initialize the session.
