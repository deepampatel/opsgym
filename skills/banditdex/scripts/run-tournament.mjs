...
<head>
  ...
  <title>BanditDex Report - ${runId}</title>
  <style>
    :root {
      color-scheme: light;
      --ink: #17211f;
...
async function main() {
  ...
  const workspace = args.workspace || ".banditdex";
  ...
  const winner = scoreboard[0];
  console.log(`Winner: ${winner.policyName} (${winner.averages.banditScore.toLocaleString("en-IN")} BanditScore)`);
