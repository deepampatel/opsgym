import { createAdapterDescriptor, BANDITDEX_SCHEMA_VERSION } from "./contracts.mjs";
...
function winsByRollout(allRollouts, policies) {
  ...
  for (const rollout of allRollouts) {
    ...
    if (!current || rollout.metrics.banditScore > current.metrics.banditScore) byIndex.set(rollout.rolloutIndex, rollout);
  }
...
export function createDemoArenaAdapter(config) {
  ...
  const metricCatalog = [
    ...
    { id: "banditScore", label: "BanditScore", unit: "count", description: "Composite score used for tournament ranking." }
  ];
  ...
  return {
    ...
    runPolicy({ environment, policyKey, policyConfig, rolloutIndex, seed }) {
      ...
      return {
        ...
        metrics: {
          ...
          banditScore: Math.round(banditScore)
        },
        daily
      };
    },
    ...
    summarizeScoreboard({ allRollouts, policies: customPolicies = policies }) {
      const scoreboard = Object.keys(customPolicies)
        ...
        .sort((a, b) => b.averages.banditScore - a.averages.banditScore);
      return {
        scoreboard,
