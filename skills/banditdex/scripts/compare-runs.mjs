...
function choosePrimaryMetric(deltas) {
  if (deltas.banditScore) return "banditScore";
...
function renderMarkdown(comparison) {
  ...
  return [
    `# BanditDex Comparison: ${comparison.comparisonId}`,
    "",
...
<head>
  ...
  <title>BanditDex Comparison - ${comparison.comparisonId}</title>
  <style>
...
<body>
  <main>
    <header>
      <p>BanditDex / ${comparison.arenaId}</p>
...
async function main() {
  ...
  if (!baselineRunId || !candidateRunId) {
    throw new Error("Usage: banditdex compare --baseline <run-id> --candidate <run-id> [--comparison <id>]");
  }
